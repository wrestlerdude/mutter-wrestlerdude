...
static void
reload_vrr_supported (MetaWindow    *window,
                      MetaPropValue *value,
                      gboolean       initial)
{
  ...
  if (value->type != META_PROP_VALUE_INVALID)
    vrr_supported = (guint32) value->v.cardinal;

  meta_window_set_vrr_supported (window, vrr_supported);
}
...
meta_x11_display_init_window_prop_hooks (MetaX11Display *x11_display)
{
  ...
  MetaWindowPropHooks hooks[] = {
    ...
    { x11_display->atom__VARIABLE_REFRESH, META_PROP_VALUE_CARDINAL, reload_vrr_supported, LOAD_INIT },
    { 0 },
  };
